...
import { Ticket, Globe, Zap, Music, MapPin, Menu, X, Calendar, Play, ChevronLeft, ChevronRight, ShieldCheck, AlertCircle } from 'lucide-react';
...
const TICKET_TYPES = [
  { name: 'Day Pass', price: '$149', color: 'white', accent: 'bg-white/5', perks: ['General Entry', 'All Stages'] },
  { name: 'Weekend', price: '$349', color: 'teal', accent: 'bg-[#4fb7b3]/10 border-[#4fb7b3]/50', perks: ['General Entry', 'All Stages', 'Expedited Entry'] },
  { name: 'Astral VIP', price: '$899', color: 'periwinkle', accent: 'bg-[#637ab9]/10 border-[#637ab9]/50', perks: ['General Entry', 'All Stages', 'Expedited Entry', 'Backstage Lounge'] },
];
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      } else if (confirmingIndex !== null) {
        if (e.key === 'Escape') setConfirmingIndex(null);
      }
    };
    window.addEventListener('keydown', handleKeyDown);
    return () => window.removeEventListener('keydown', handleKeyDown);
  }, [selectedArtist, confirmingIndex]);
      ...
      <AnimatePresence>
        {confirmingIndex !== null && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md cursor-auto"
          ...
          >
            <motion.div
              ...
              animate={{ scale: 1, y: 0 }}
              exit={{ scale: 0.9, y: 20 }}
              className="relative w-full max-w-lg bg-[#1a1b3b] border-2 border-[#4fb7b3]/50 p-8 md:p-12 shadow-[0_0_50px_rgba(79,183,179,0.2)]"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="flex flex-col items-center text-center">
                <div className="w-16 h-16 rounded-full bg-[#4fb7b3]/10 flex items-center justify-center mb-6">
                  <ShieldCheck className="w-8 h-8 text-[#4fb7b3]" />
                </div>
                
                <h3 className="text-3xl font-heading font-bold uppercase mb-2">Secure Link</h3>
                <p className="text-gray-400 text-sm mb-8 font-mono tracking-tight">Confirming frequency allocation...</p>
                
                <div className="w-full bg-white/5 rounded-xl p-6 mb-8 border border-white/10">
                   <div className="flex justify-between items-center mb-2">
                      <span className="text-gray-400 text-xs uppercase tracking-widest">Selected Tier</span>
                      <span className="font-bold text-[#a8fbd3]">{TICKET_TYPES[confirmingIndex].name}</span>
                   </div>
                   <div className="flex justify-between items-center border-t border-white/10 pt-4 mt-2">
                      <span className="text-gray-400 text-xs uppercase tracking-widest">Total Frequency Cost</span>
                      <span className="text-2xl font-bold">{TICKET_TYPES[confirmingIndex].price}</span>
                   </div>
                </div>
                ...
                <div className="flex flex-col gap-4 w-full">
                  <button 
                    ...
                    className="w-full py-4 bg-[#4fb7b3] text-black font-bold uppercase tracking-widest hover:bg-[#a8fbd3] transition-colors"
                    data-hover="true"
                  >
                ...
                <div className="mt-8 flex items-center gap-2 text-[10px] text-white/30 font-mono uppercase">
                   <AlertCircle className="w-3 h-3" />
                   <span>Transmissions are irreversible. Terms apply.</span>
                </div>
              </div>
            </motion.div>
          </motion.div>
